...
| — | `#pragma pack(n)` / `#pragma pack()` | Packed struct layout (see `structs.md`) |

`@alias` can be applied to native functions, native structs, struct fields, and struct methods. See `serialization.md` for `@serializable`.
